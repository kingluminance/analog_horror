...
**탭 3개가 전부 하나의 테두리 선을 공유하는 대신, 각자 자기만의 강조선("종이 조각")을 가짐**
("인덱스에 맞춰서 줄도 3개" → "짧은 밑줄 말고 길게, 종이 3장이 겹친 것처럼" 두 차례 피드백) --
각 슬롯에 `ColorRect`를 버튼보다 먼저 추가해서(버튼이 나중에=위에 그려져서 겹치는 부분은 가려지고,
나머지가 버튼 아래로 길게 삐져나와 보임) 선택된 탭은 밝은 금색·나머지는 어두운 호박색으로 칠함
(`_tab_underlines`, `_update_tab_buttons()`에서 매번 색만 갱신). 처음엔 4px짜리 얇은 밑줄이었는데
...
버튼/밑줄은 `RANK_OFFSETS`의 **x만** 쓰고 y는 예전처럼 `SLOT_HEIGHT` 기준으로 전부 같은
