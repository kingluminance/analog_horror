...
**탭 3개가 전부 하나의 테두리 선을 공유하는 대신, 각자 자기만의 짧은 강조선(밑줄)을 가짐**
("인덱스에 맞춰서 줄도 3개" 요청) -- 각 슬롯에 `ColorRect` 밑줄을 버튼보다 먼저 추가해서
(버튼이 나중에=위에 그려져서 겹치는 1px 정도는 버튼 밑에 가려지고, 나머지 몇 px만 버튼 아래로
삐져나와 보임), 선택된 탭은 밝은 금색·나머지는 어두운 호박색으로 칠함(`_tab_underlines`,
`_update_tab_buttons()`에서 매번 색만 갱신).
